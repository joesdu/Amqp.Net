...
public sealed class SaslMechanisms
{
    ...
    public int Encode(Span<byte> buffer)
    {
        ...
        int bodySize = AmqpEncoder.EncodeSymbolArray(body, Mechanisms);
...
public sealed class SaslInit
{
    ...
    private int GetFieldCount()
    {
        ...
        if (InitialResponse != null) return 2;
        return 1;
    }
    ...
    public static SaslInit Decode(ReadOnlySpan<byte> buffer, out int bytesConsumed)
    {
        ...
        // Field 0: mechanism
        string mechanism = AmqpDecoder.DecodeSymbol(buffer[offset..], out int consumed);
        offset += consumed;
    ...
    public static SaslInit CreatePlain(string username, string password, string? hostname = null)
    {
        ...
        response[0] = 0;
        System.Text.Encoding.UTF8.GetBytes(username, response.AsSpan(1));
        response[1 + username.Length] = 0;
        System.Text.Encoding.UTF8.GetBytes(password, response.AsSpan(2 + username.Length));
    ...
    /// </summary>
    public static SaslInit CreateAnonymous(string? hostname = null) => new()
    {
...
public sealed class SaslChallenge
{
    ...
    public int Encode(Span<byte> buffer)
    {
        ...
        Span<byte> body = stackalloc byte[256];
        int bodySize = AmqpEncoder.EncodeBinary(body, Challenge);
...
public sealed class SaslResponse
{
    ...
    public int Encode(Span<byte> buffer)
    {
        ...
        Span<byte> body = stackalloc byte[256];
        int bodySize = AmqpEncoder.EncodeBinary(body, Response);
...
public sealed class SaslOutcome
{
    ...
    public int Encode(Span<byte> buffer)
    {
        ...
        int fieldCount = AdditionalData != null ? 2 : 1;
        Span<byte> body = stackalloc byte[256];
        int bodySize = 0;
    ...
    public static SaslOutcome Decode(ReadOnlySpan<byte> buffer, out int bytesConsumed)
    {
        ...
        // Field 0: code
        byte code = AmqpDecoder.DecodeUByte(buffer[offset..], out int consumed);
        offset += consumed;
