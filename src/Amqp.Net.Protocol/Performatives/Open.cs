...
public sealed record Open : PerformativeBase
{
    ...
    private static int EncodeSymbolArray(Span<byte> buffer, string[]? values)
    {
        ...
        bool useSmall = values.All(v => v.Length <= 255);
        ...
        foreach (var value in values)
        {
            if (useSmall)
            {
                ...
                System.Text.Encoding.ASCII.GetBytes(value, buffer[offset..]);
                offset += value.Length;
            }
            else
            {
                System.Buffers.Binary.BinaryPrimitives.WriteUInt32BigEndian(buffer[offset..], (uint)value.Length);
                offset += 4;
                System.Text.Encoding.ASCII.GetBytes(value, buffer[offset..]);
                offset += value.Length;
            }
    ...
    private static int EncodeProperties(Span<byte> buffer, IReadOnlyDictionary<string, object?>? properties)
    {
        return AmqpEncoder.EncodeMap(buffer, properties);
    }

    private int GetFieldCount()
    {
        // Count non-null trailing fields
        if (Properties != null && Properties.Count > 0) return 10;
        if (DesiredCapabilities != null && DesiredCapabilities.Length > 0) return 9;
        if (OfferedCapabilities != null && OfferedCapabilities.Length > 0) return 8;
        if (IncomingLocales != null && IncomingLocales.Length > 0) return 7;
        if (OutgoingLocales != null && OutgoingLocales.Length > 0) return 6;
        if (IdleTimeOut.HasValue) return 5;
        if (ChannelMax != ushort.MaxValue) return 4;
        if (MaxFrameSize != uint.MaxValue) return 3;
    ...
    public override int GetEncodedSize()
    {
        ...
        return GetDescriptorSize(DescriptorCode) + GetListHeaderSize(GetMaxBodySize(), GetFieldCount()) + GetMaxBodySize();
    }
    ...
    public static Open Decode(ReadOnlySpan<byte> buffer, out int listSize, out int bytesConsumed)
    {
        ...
        // Field 0: container-id (mandatory)
        string containerId = AmqpDecoder.DecodeString(buffer[offset..], out int consumed);
        offset += consumed;

        string? hostname = null;
        uint maxFrameSize = uint.MaxValue;
        ushort channelMax = ushort.MaxValue;
    ...
    {
        return $"Open(ContainerId={ContainerId}, Hostname={Hostname}, MaxFrameSize={MaxFrameSize}, ChannelMax={ChannelMax}, IdleTimeOut={IdleTimeOut})";
    }
}
