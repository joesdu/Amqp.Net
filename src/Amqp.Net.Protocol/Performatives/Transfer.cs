...
public sealed record Transfer : PerformativeBase
{
    ...
    {
        if (Batchable) return 11;
        if (Aborted) return 10;
        if (Resume) return 9;
        if (State != null) return 8;
        if (RcvSettleMode.HasValue) return 7;
        if (More) return 6;
        if (Settled.HasValue) return 5;
        if (MessageFormat.HasValue) return 4;
        if (DeliveryTag != null) return 3;
        if (DeliveryId.HasValue) return 2;
        return 1;
    }
    ...
    public static Transfer Decode(ReadOnlySpan<byte> buffer, out int listSize, out int bytesConsumed)
    {
        ...
        if (count <= 2) return transfer with { DeliveryId = deliveryId };
        ...
        if (count <= 3) return transfer with { DeliveryId = deliveryId, DeliveryTag = deliveryTag };
        ...
        if (count <= 4) return transfer with { DeliveryId = deliveryId, DeliveryTag = deliveryTag, MessageFormat = messageFormat };
        ...
        if (count <= 5) return transfer with { DeliveryId = deliveryId, DeliveryTag = deliveryTag, MessageFormat = messageFormat, Settled = settled };
    ...
    public override string ToString() =>
        $"Transfer(Handle={Handle}, DeliveryId={DeliveryId}, Settled={Settled}, More={More})";
}
