...
public sealed record Attach : PerformativeBase
{
    ...
    public static Attach Decode(ReadOnlySpan<byte> buffer, out int listSize, out int bytesConsumed)
    {
        ...
        // Field 0: name
        var name = AmqpDecoder.DecodeString(buffer[offset..], out var consumed);
        offset += consumed;

        // Field 1: handle
        var handle = AmqpDecoder.DecodeUInt(buffer[offset..], out consumed);
        offset += consumed;

        // Field 2: role
        var role = AmqpDecoder.DecodeBoolean(buffer[offset..], out consumed);
        offset += consumed;
        ...
        if (count <= 4)
        {
            return attach with { SndSettleMode = sndSettleMode };
        }
    ...
    /// <inheritdoc />
    public override string ToString() => $"Attach(Name={Name}, Handle={Handle}, Role={(Role ? "receiver" : "sender")})";
}
...
public sealed class Source
{
    ...
    public int Encode(Span<byte> buffer)
    {
        ...
        var fieldCount = Address != null ? 1 : 0;
