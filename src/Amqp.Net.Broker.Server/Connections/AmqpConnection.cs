...
public sealed class AmqpConnection : IAsyncDisposable
{
    ...
    private ConnectionState _state = ConnectionState.Start;
    private Open? _remoteOpen;
    private Open? _localOpen;
    private uint _negotiatedMaxFrameSize;
    private ushort _negotiatedChannelMax;
        ...
        IBrokerLinkHandler? linkHandler = null)
    {
        ...
        _negotiatedMaxFrameSize = options.MaxFrameSize;
        _lastActivity = DateTimeOffset.UtcNow;

        _frameReader = new FrameReader(context.Input, options.MaxFrameSize);
        _frameWriter = new FrameWriter(context.Output, options.MaxFrameSize);
    }
    ...
    /// </summary>
    public Open? RemoteOpen => _remoteOpen;

    /// <summary>
    /// Gets the local Open performative (after connection is opened).
    /// </summary>
    public Open? LocalOpen => _localOpen;

    /// <summary>
    /// Gets the negotiated maximum frame size.
    /// </summary>
    public uint MaxFrameSize => _negotiatedMaxFrameSize;

    /// <summary>
    /// Gets the negotiated channel maximum.
    /// </summary>
    public ushort ChannelMax => _negotiatedChannelMax;
    ...
    /// </summary>
    internal FrameWriter FrameWriter => _frameWriter;

    /// <summary>
    /// Gets the broker link handler for internal use.
    /// </summary>
    internal IBrokerLinkHandler? LinkHandler => _linkHandler;
    ...
    public async Task RunAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _state = ConnectionState.HeaderExchanged;
            Log.ConnectionStateChanged(_logger, ConnectionId, _state);

            // Main frame processing loop
            while (!token.IsCancellationRequested && !_state.IsTerminal())
            {
    ...
    {
        if (_state != ConnectionState.HeaderExchanged && _state != ConnectionState.OpenSent)
        {
            throw new AmqpConnectionException($"Received Open in invalid state: {_state}");
        }
        ...
        _negotiatedMaxFrameSize = Math.Min(_options.MaxFrameSize, open.MaxFrameSize);
        _negotiatedChannelMax = Math.Min((ushort)65535, open.ChannelMax);
        
        // Negotiate idle timeout (use minimum of both, 0 means disabled)
        uint localTimeout = _options.IdleTimeoutMs;
        uint remoteTimeout = open.IdleTimeOut ?? 0;
        if (localTimeout == 0)
        ...
        await _frameWriter.WriteFrameAsync(0, _localOpen, cancellationToken).ConfigureAwait(false);
        Log.OpenSent(_logger, ConnectionId, _localOpen.ContainerId);
        ...
        Log.ConnectionOpened(_logger, ConnectionId, _negotiatedMaxFrameSize, _negotiatedChannelMax, _negotiatedIdleTimeout);
    }
    ...
    private async Task HandleCloseAsync(Close close, CancellationToken cancellationToken)
    {
        ...
        if (_state == ConnectionState.ClosePipe)
        {
        ...
        {
            // Send Close response
            _state = ConnectionState.CloseReceived;
            
            var response = new Close();
            await _frameWriter.WriteFrameAsync(0, response, cancellationToken).ConfigureAwait(false);
    ...
    private async Task HandleBeginAsync(ushort channel, Begin begin, CancellationToken cancellationToken)
    {
        ...
        {
            throw new AmqpConnectionException($"Received Begin in invalid connection state: {_state}");
        }

        // Create new session
        var session = new AmqpSession(this, channel, _frameWriter, _logger);
    ...
    private void StartIdleTimer()
    {
        ...
        _idleTimer = new Timer(OnIdleTimerTick, null, interval, interval);
    }
    ...
    private async void OnIdleTimerTick(object? state)
    {
        try
        {
            ...
            // Send heartbeat
            await _frameWriter.WriteHeartbeatAsync(0, _connectionCts.Token).ConfigureAwait(false);
    ...
    /// </summary>
    public Task CloseAsync(string? errorCondition = null, string? errorDescription = null)
    {
        return CloseInternalAsync(errorCondition, errorDescription, _connectionCts.Token);
    }

    private async Task CloseInternalAsync(string? errorCondition, string? errorDescription, CancellationToken cancellationToken)
    {
        if (_state.IsTerminal() || _state.IsClosing())
        {
            return;
        }

        _state = ConnectionState.ClosePipe;
        Log.ConnectionStateChanged(_logger, ConnectionId, _state);
        ...
        await _frameWriter.WriteFrameAsync(0, close, cancellationToken).ConfigureAwait(false);
        Log.CloseSent(_logger, ConnectionId);
    }
